...
import static org.xiao.ns.mapper.AppDynamicSqlSupport.*;
...
@Mapper
public interface AppMapper extends CommonCountMapper, CommonDeleteMapper, CommonInsertMapper<App>, CommonUpdateMapper {
    ...
    default int insertMultiple(Collection<App> records) {
        return MyBatis3Utils.insertMultiple(this::insertMultiple, records, app, c ->
            c.map(id).toProperty("id")
            .map(code).toProperty("code")
            .map(name).toProperty("name")
            .map(createBy).toProperty("createBy")
